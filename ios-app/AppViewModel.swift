...
final class AppViewModel: ObservableObject {
    ...
    func startCardScanning() {
        ...
        // AirTraffic exports are move operations. Even restoring the same bytes
        // creates a new inode and invalidates passd's live database handle, so
        // Wallet metadata files must never be touched by card discovery.
        startLegacyCardScanning(fallbackReason: "safe live-log mode; Wallet files are not accessed")
    }
    ...
    func flashCards() {
        ...
        Task.detached { [weak self] in
            ...
            for (i, card) in selected.enumerated() {
                ...
                await MainActor.run {
                    self.cardFlashLog.append("  ✅ Artwork transfer completed. Invalidating pass cache…")
                }
